...
public class StartNewGameWindow extends JFrame {
    ...
    private void addGameControlFieldWinLength() {
        ...
        final int[] currentFieldSize = new int[1];
        add(new JLabel("Choose field size:"));
        JLabel lblFieldsSize = new JLabel(STR_FIELD_SIZE + MIN_FIELD_SIZE);
        add(lblFieldsSize);

        slFieldSize = new JSlider(MIN_FIELD_SIZE, MAX_FIELD_SIZE, MIN_FIELD_SIZE);
        currentFieldSize[0] = MIN_WIN_LEN;
        slFieldSize.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                currentFieldSize[0] = slFieldSize.getValue();
                lblFieldsSize.setText(STR_FIELD_SIZE + currentFieldSize[0]);
                slWinLength.setMaximum(currentFieldSize[0]);
            }
        });
        add(slFieldSize);
